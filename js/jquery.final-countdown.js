...
(function ($) {
    ...
    function dispatchTimer() {
        var currentTime=settings.now.getTime();
        var eventTime=settings.end.getTime();
        var remTime=eventTime-currentTime;
        var s=Math.floor(remTime/1000);
        var m=Math.floor(s/60);
        var h=Math.floor(m/60);
        var d=Math.floor(h/24);
        var t=Math.floor((settings.end.getTime()-settings.start.getTime())/1000);
        h%=24;
        s%=60;
        m%=60;
        timer={
    ...
    function startCounters() {        
        var interval = setInterval( function() {                        
                ...
                if ( timer.minutes == 0 && timer.hours == 0 && timer.days == 0 && timer.seconds==0) {
